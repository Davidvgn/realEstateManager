...
class GetLoanYearlyAndMonthlyPaymentUseCase
    @Inject
    constructor() {
        fun invoke(
            loanAmount: BigDecimal,
            loanInterest: BigDecimal,
            loanDuration: BigDecimal,
            ...
            val numerator =
                monthlyInterestRate.multiply(
                    (BigDecimal.ONE + monthlyInterestRate).pow(
                        loanDurationInMonths.toInt(),
                    ),
                )
            val denominator =
                (BigDecimal.ONE + monthlyInterestRate).pow(loanDurationInMonths.toInt()) - BigDecimal.ONE

            val monthlyPayment: BigDecimal =
                loanAmount.multiply(numerator).divide(denominator, 2, RoundingMode.HALF_UP)
